...
import { KANBAN_COLUMNS } from '../data/projectTemplates';
...
export const CommercialView = ({ leads, onAddLead, onUpdateLead }: CommercialViewProps) => {
  ...
  const handleAddInteraction = () => {
      ...
      const updatedLead = {
          ...selectedLead,
          lastContact: interactionDate, // Update last contact
          history: [newHistoryItem, ...selectedLead.history] // Add to top (reverse chronological)
      };

      onUpdateLead(updatedLead);
      setSelectedLead(updatedLead); // Update local selection to show immediate change
      setInteractionNote(''); // Reset form
  };
            ...
            <div className="border-t pt-4 mb-4">
                ...
                <div className="space-y-3 bg-gray-50 p-3 rounded border border-gray-200">
                    <Input 
                        ...
                        onChange={(e: any) => setInteractionDate(e.target.value)} 
                    />
                    <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">Observação / Resumo</label>
                        <textarea 
                            className="w-full border border-gray-300 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#ff9933]" 
                            rows={3}
                            value={interactionNote}
                            onChange={(e) => setInteractionNote(e.target.value)}
                            placeholder="Ex: Reunião de alinhamento, cliente solicitou desconto..."
                        ></textarea>
                    </div>
                    <Button onClick={handleAddInteraction} className="w-full text-sm" disabled={!interactionNote}>
                        <Save size={14} /> Registrar Histórico
                    </Button>
                </div>
            </div>
