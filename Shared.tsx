...
export const Input = ({ label, type = "text", value, onChange, placeholder, multiline = false }: any) => (
  <div className="mb-4">
    ...
    {multiline ? (
      <textarea className="w-full border border-gray-300 rounded-md px-3 py-2 focus:ring-2 focus:ring-[#ff9933] outline-none" rows={3} value={value} onChange={onChange} placeholder={placeholder} />
    ) : (
      <input type={type} className="w-full border border-gray-300 rounded-md px-3 py-2 focus:ring-2 focus:ring-[#ff9933] outline-none" value={value} onChange={onChange} placeholder={placeholder} />
    )}
  </div>
);
